...
function CourtBooking() {
  ...
  const inititeBooking  = async() =>{
    ...
    // async function displayRazorpay() {
      ...
      const result = await AxiosInstance.post("/payment/orders",{slotId:SelectedSlots._id});
...
return (
    <>
      <div>
        ...
        <div className="courtname_addtimeslot">
          ...
          <div>
           {userDetails.role===1 &&  <Button variant="primary" onClick={handleShow}>
              Add Time Slot
            </Button>}
        ...
        <div className="courttimedisplay">
          ...
          <div>
            ...
            {slotData.map((slot) => (
             ...
             <span key={slot.id} style={{ margin:'5px', padding:'1px',  }} onClick={() => { !slot.bookedBy && setusershow(true); !slot.bookedBy && setSelectedSlots(slot)}}> 
              <button className={`${slot.bookedBy?'bg-warning' : 'bg-primary'}`}>{slot.slot.name}</button>
    ...
    <span
      key={slot.id}
      style={{ border: '1px solid red', display: 'inline-block', background: '#00FFFF' }}
      onClick={() => {
        setusershow(true);
        setSelectedSlots(slot);
      }}
    >
      <button>{slot.slot.name}</button>
    </span>
  ))} */}
                ...
                <Modal show={usershow} onHide={userHandleClose}>
                  ...
                  {/* <form onSubmit={handilCreateTimeSlot}> */}
                   ...
                   <li>Amount: {SelectedSlots?.cost}</li> 
                    ...
                    <li>date: {SelectedSlots?.date ? new Date(SelectedSlots.date).toLocaleDateString('en-GB') : 'No date selected'}</li>

                    <button type="submit" className="btn btn-primary"  style={{ width: '25%', margin: '0 auto', display: 'block' }} onClick={inititeBooking}>Book Now</button>
                    ...
                    <Modal.Footer>
                      ...
                      {/* <button onClick={userHandleClose}> * </button> */}
                    </Modal.Footer>
                  {/* </form> */}
                </Modal>
              </div>
